...
void log_cgroup(cgroup_data_t *cgroup_data, job_id_info_t *job_info, demeter_conf_t *conf)
{
    ...
    if (cgroup_data == NULL || job_info == NULL || conf == NULL)
        return;
    sprintf(log_msg, "\nCgroup data for job %u:\n\tmem_max_usage_bytes: %u\n\toom_kill_disable: %d\n\tunder_oom: %d\n\toom_kill: %d\n\tcpuset_cpus: %s\n\tcpuset_effective_cpus: %s\n", job_info->job_id ,cgroup_data->mem_max_usage_bytes, cgroup_data->oom_kill_disable, cgroup_data->under_oom, cgroup_data->oom_kill, cgroup_data->cpuset_cpus, cgroup_data->cpuset_effective_cpus);
